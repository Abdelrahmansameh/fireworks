...
function buildDroneMesh() {
    const vertices = new Float32Array([
        ...
        -0.8,   0.0,  // 3 – tail
        -0.55, -0.18, // 4 – lower back shoulder
         0.05, -0.45, // 5 – lower wing tip
    ]);
...
// CPU buffer layout (strideFloats = 17)
// 0:px  1:py  2:vx  3:vy  4:targX  5:targY  6:wanderTimer
// 7:lifetime  8:initialLifetime  9:(pad)  10:radius
// 11:rotation  12:speed  13:r  14:g  15:b  16:a
...
class InstancedDroneSystem {
    ...
    update(delta, onAwardSparkles) {
        ...
        for (let i = 0; i < this.count; i++) {
            ...
            if (distToTarget < 80 || d[base + this.WANDER_TIMER] <= 0) {
                const wt = this._randomWanderTarget();
                d[base + this.TX] = wt.x;
                d[base + this.TY] = wt.y;
            ...
            // Smooth velocity steering toward target
            const speed = d[base + this.SPEED];
            const invDist = distToTarget > 0.01 ? 1 / distToTarget : 0;
            const steerX = dx * invDist * speed;
            const steerY = dy * invDist * speed;
            const steerK = 1 - Math.exp(-5 * delta);
            d[base + this.VX] += (steerX - d[base + this.VX]) * steerK;
            d[base + this.VY] += (steerY - d[base + this.VY]) * steerK;
